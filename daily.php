...
<head>
    ...
    <a href="https://www.earleco.com"> Walter Earle / </a> <a href="https://portal.earleco.com/Account/Login?ReturnUrl=%2f"> Employee Portal / </a><a href="login.php"> Log Out</a>
...
$sql = "SELECT CustId, workd, Cust, JobInfo, Forman, Phone, Material, MixCode, Tonnage, safe, tonsOut, Time, Trucks, Plant FROM dayorders";
    ...
    echo("<table>");
    echo("<tr><th>Work Date</th><th>Customer</th><th>Job Information</th><th>Forman</th><th>Phone</th><th>Material</th><th>Mix Code</th><th>Tonnage</th><th>Safe #</th><th>Tons Out</th><th>Load Time</th><th>Trucks</th><th>Plant</th></tr>");
...
echo("</fieldset>");
print("<br/>");
$sql = "SELECT custId, workd, cust, jobInfo, forman, phone, material, mixCode, tonnage, time, trucks, plant FROM nightorders";
    $result = $connection -> query($sql);
    echo("<fieldset>");
    echo("<legend>WRE Jackson Night Shift Orders</legend>");
    echo("<table>");
    echo("<tr><th>Work Date</th><th>Customer</th><th>Job Information</th><th>Forman</th><th>Phone</th><th>Material</th><th>Mix Code</th><th>Tonnage</th><th>Load Time</th><th>Trucks</th><th>Plant</th></tr>");
    $row = mysqli_fetch_array($result);
    while($row = mysqli_fetch_array($result)){
    echo "<tr>";
    echo "<td>" . $row['workd'] . "</td>" .
...
$sql = "SELECT truckId, tCust, jInfo, jContact, tTruck, notes FROM truckorders";
    ...
    "<td>" . $row['tTruck'] . "</td>" .
    "<td>" . $row['notes'] . "</td>" ;
    //"<td><a href='edb1.1tc.php?truckId=" . $row['truckId'] . "'></a></td>" . 
    //"<td><a href='edb1.1td.php?truckId=" . $row['truckId'] . "'></a></td>";
    echo "</tr>";
}
echo("</table>");
print("<a href='edb1.1ta.php'></a>");
echo("</fieldset>");
print("<br/>");
$sql = "SELECT oilId, aload, dMat, dLoad, dPlant, pLoad, nMat, nLoad, nPlant FROM oilorders";
    ...
    "<td>" . $row['nLoad'] . "</td>" .
    "<td>" . $row['nPlant'] . "</td>" ;
   // "<td><a href='edb1.1oc.php?oilId=" . $row['oilId'] . "'></a></td>" . 
   // "<td><a href='edb1.1od.php?oilId=" . $row['oilId'] . "'></a></td>";
    echo "</tr>";
}
echo("</table>");
